...
two signals. We check if we lost fix, by counting how long passes between PPS, 
and only output data if there is a fix, otherwise waiting to get a fix again. 
...
void setup() {
  ...
  //Set up Interrupt from FIX:
  PORTB.DIRCLR = PIN2_bm; //pin PB2
...
void checkFix(){
  if (overflowCounter * TCB2.CCMP * 62.5 > 1.2*1000000000){ 
      //we counted more than 1.1 second since last PPS, assume we lost fix
      Serial.print("GPS Fix lost at;");
      printTime(second,overflowCounter * TCB2.CCMP *62.5,true);
      Serial.println("");
      GPS.day=0; //note that we do not know time
      startup=true; //reset everything
      Ready=false;
  }
}
...
void CheckPulse(){
  ...
  if ((Start==true) and (Stop ==true)){
    ...
    if (Difference < maxTimeDiff and Difference > minTimeDiff){
      ...
      printTime(EventSec,EventNanosec,true);
...
void printTime(unsigned long sec,unsigned long nanosec, bool printDate){
  ...
  int PrintHour=(hour+(minute+(sec+nanosec/1000000000)/60)/60)%24;
  int PrintDay=GPS.day+(hour+(minute+(sec+nanosec/1000000000)/60)/60)/24;
  ...
  if (printDate){
    Serial.print(";Date:");
    Serial.print(PrintDay, DEC); Serial.print('/');
